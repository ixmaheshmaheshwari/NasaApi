...
const Earth = () => {
  ...
  useEffect(() => {
    ...
    fetchedData(apiUrl, apiKey); // Call fetchData from your custom hook'
    setImageUrl(`https://api.nasa.gov/planetary/earth/imagery?lon=100.75&lat=1.5&date=2022-01-01&dim=0.15&api_key=${apiKey}`)
  ...
  <div className="border border-gray-300 rounded p-4 shadow-md mb-4 m-5 earthView">
    <div className="mb-2">
      <span className="font-semibold">DataSet of Image:</span> {defaultData.resource.dataset} 
    </div>
    <div className="mb-2">
      <span className="font-semibold">Planet:</span> {defaultData.resource.planet}
    </div>
    <div className="mb-2">
      <span className="font-semibold">Service Version:</span> {defaultData.service_version}
    </div>
    {imageUrl ? (
