...
const Epic = () => {
  ...
  async function fetchDataNaturalImage() {
   ...
   await axios.get(`https://api.nasa.gov/EPIC/api/natural?api_key=${apiKey}`)
      .then(response => {
  ...
  function fetchDataEnhancedImage() {
    ...
    axios.get(`https://api.nasa.gov/EPIC/api/enhanced?api_key=${apiKey}`)
      .then(response => {
 ...
 <ProgressiveImage src={`https://api.nasa.gov/EPIC/archive/natural/${formatDateFromIdentifier(image.identifier)}/png/${image.image}.png?api_key=${apiKey}`} placeholder={loader}>
                {(src, loading) => (
                  <div className="relative w-full h-auto">
                    <img src={loading ? loader : src} alt={image.caption} className={`w-full h-auto ${loading ? 'w-16 h-16' : ''}`} />
                  </div>
                )}
              </ProgressiveImage>
  ...
  <div className="p-4">
    ...
    <p className="text-sm text-gray-600">Location: {image.centroid_coordinates.lat}, {image.centroid_coordinates.lon}</p>
  </div>
</div>
      ...
      <div className='flex flex-col items-center mt-50'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
          {enhancedData.map(image => (
            <div className="bg-white rounded-lg shadow-lg overflow-hidden m-2" key={image.identifier}>
              <ProgressiveImage delay={3000} src={`https://api.nasa.gov/EPIC/archive/enhanced/${formatDateFromIdentifier(image.identifier)}/png/${image.image}.png?api_key=${apiKey}`} placeholder={loader}>
                {(src, loading) => (
                  <div className="relative w-full h-auto">
                    <img src={loading ? loader : src} alt={image.caption} className={`w-full h-auto ${loading ? 'w-16 h-16' : ''}`} />
                  </div>
  ...
  return (
    ...
    <div className='lex justify-center items-center  ' style={{backgroundColor:"#dad7cd",  minHeight: "100vh"}}>
  ...
  <div className="max-w-md w-full bg-gray-100 rounded-lg overflow-hidden shadow-lg p-6">
    <p className="text-lg text-gray-800">"Time travel used to be thought of as just science fiction, but Einstein's general theory of relativity allows for the possibility that we could warp space-time so much that you could go off in a rocket and return before you set out."</p>
    <p className="text-gray-600 text-right mt-4">- Stephen Hawking</p>
  </div>

  <img src={rocket} alt="Rocket" class='h-120 w-110 mx-4' />

  <div className="max-w-md w-full bg-gray-100 rounded-lg overflow-hidden shadow-lg p-6">
    <p className="text-lg text-gray-800">"Space is big. You just won't believe how vastly, hugely, mind-bogglingly big it is. I mean, you may think it's a long way down the road to the drug store, but that's just peanuts to space."</p>
